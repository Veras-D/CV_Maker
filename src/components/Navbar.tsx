...
import { Download, DownloadCloud, UploadCloud, CheckCircle2 } from 'lucide-react';
import { exportCVToPDF } from '../utils/pdfExport';
import { ProModal } from './Common/ProModal';
import { NavWorkspaceTabs } from './NavWorkspaceTabs';
  ...
  return (
    <header className="bg-slate-950 border-b border-slate-800/80 text-slate-100 w-full relative">
      <div className="max-w-7xl mx-auto px-4 py-1.5 flex items-center justify-between gap-3">
        ...
        <NavWorkspaceTabs activeTab={activeTab} onSelectTab={setActiveTab} />
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            className="p-1.5 text-slate-400 hover:text-sky-400 hover:bg-slate-900 rounded-lg transition-colors hidden md:block cursor-pointer"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-sky-600 hover:bg-sky-500 text-white px-3 py-1 rounded-md text-xs font-semibold shadow-sm transition-all cursor-pointer"
          >
      ...
      {feedbackNotification && (
        <div className="bg-sky-950 border-b border-sky-800 text-sky-200 text-xs px-4 py-1.5 flex items-center justify-between font-medium shadow-inner animate-in fade-in duration-200">
          ...
          <button onClick={() => setFeedbackNotification(null)} className="text-sky-300 hover:text-white text-xs cursor-pointer">
            ✕
          </button>
        </div>
      )}
